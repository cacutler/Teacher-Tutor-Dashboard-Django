...
def course_detail(request, pk):
    course = get_object_or_404(Course.objects.select_related("program", "program__teacher").prefetch_related("assignments"), pk=pk)
    return render(request, "core/course_detail.html", {"course": course})
@login_required
def assignment_detail(request, pk):
    assignment = get_object_or_404(Assignment.objects.select_related("course", "teacher"), pk=pk)
    ...
    if request.method == "POST" and not is_teacher:
        ...
        if entry:
            submission, _ = Submission.objects.update_or_create(assignment=assignment, student=request.user, defaults={"entry": entry})
            return redirect("assignment_detail", pk=pk)
    context = {
        "assignment": assignment,
        "is_teacher": is_teacher,
        "my_submission": my_submission,
        "submissions": (assignment.submissions.select_related("student").all() if is_teacher else None)
    }
