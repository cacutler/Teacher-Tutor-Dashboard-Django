...
class CourseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Course
        fields = ["id", "program", "program_title", "teacher", "title", "subject", "description", "prerequisites"]
class AssignmentSerializer(serializers.ModelSerializer):
    teacher = MinimalUserSerializer(read_only=True)
    course_title = serializers.CharField(source="course.title", read_only=True)
    class Meta:
        model = Assignment
        fields = ["id", "teacher", "course", "course_title", "title", "description"]
class SubmissionSerializer(serializers.ModelSerializer):
    student = MinimalUserSerializer(read_only=True)
    assignment_title = serializers.CharField(source="assignment.title", read_only=True)
    class Meta:
        model = Submission
        fields = ["id", "assignment", "assignment_title", "student", "entry", "submitted_at"]
...
class TutoringAppointmentSerializer(serializers.ModelSerializer):
    ...
    tutee_id = serializers.PrimaryKeyRelatedField(source="tutee", queryset=User.objects.all(), write_only=True)
    class Meta:
        model = TutoringAppointment
        fields = ["id", "tutor", "tutee", "tutee_id", "course", "title", "notes", "report"]
